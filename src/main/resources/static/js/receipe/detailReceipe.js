...
async function fetchReceipeDetails() {
    try {
        ...
        if (!response.ok) {
            ...
            const categoryLabel = getCategoryLabel(data.data.theme);
            document.querySelector('input[name="카테고리"]').value = categoryLabel;
...
function getCategoryLabel(theme) {
    const categoryMap = {
        'S': '3분 요리',
        'A': '냉텰 요리',
        'O': '원팬 요리',
    ...
    // 주어진 theme 값에 해당하는 레이블을 반환합니다.
    // 만약 theme 값이 매핑되지 않은 경우, 기본값(여기서는 빈 문자열)을 반환합니다.
